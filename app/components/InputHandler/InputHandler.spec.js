...
describe('InputHandler render', () => {
  ...
  beforeEach(() => {
    ...
    React.useRef.mockReturnValue({current: {value: text}});
  });
  ...
  it('shortcut: true, ctrlKey + Enter, send the message', () => {
    ...
    const textArea = findByTestAttr(component, 'text');
    textArea.simulate('keyDown', {keyCode: KEY_ENTER, ctrlKey: true, preventDefault});
    expect(preventDefault).toHaveBeenCalled();
    expect(addMessage).toHaveBeenCalled();
  });
  ...
  it('shortcut: true, Command + Enter, send the message', () => {
    ...
    const textArea = findByTestAttr(component, 'text');
    textArea.simulate('keyDown', {keyCode: KEY_ENTER, metaKey: true, preventDefault});
    expect(preventDefault).toHaveBeenCalled();
    expect(addMessage).toHaveBeenCalled();
  });
  ...
  it('empty text, no message sended', () => {
    const userName = 'name test';
    React.useRef = jest.fn().mockReturnValue({current: {value: ''}});
    component = setUp({addMessage, userName});
...
function setUp(props = {}) {
  const initialProps = {
    userName: '', shortcut: true, timeFormat: '', addMessage: () => {}, ...props
  }
